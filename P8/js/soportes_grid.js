...
const SoportesGrid = {
  ...
  updateKPIs: async function (fechaInicio, fechaFin) {
    ...
    facturasUnicas.forEach(fact => {
      ...
      // Solo considerar facturas en el rango seleccionado
      if (fd >= start && fd <= end) {
  ...
  renderPendingTable: function () {
    ...
    const filtrados = this.facturasPendientesGlobal.filter(f => {
      ...
      if (searchStr) {
        const fullString = `${f.factura} ${f.cliente} ${f.proveedor} ${f.lote}`.toLowerCase();
    ...
    // Sort desc by number or date
    filtrados.sort((a, b) => b.factura.localeCompare(a.factura));

    filtrados.forEach(f => {
      const row = document.createElement('tr');
      row.innerHTML = `
        <td>${f.factura}</td>
        <td>${f.cantidad.toLocaleString('es-CO')}</td>
        <td title="${f.referencia}">${f.referencia || '-'}</td>
        <td>${f.lote || '-'}</td>
        <td>
          <button class="btn-table-action" title="Ver detalles">
