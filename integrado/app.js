...
// Función para actualizar contadores de traslados
function updateTransferCounters() {
    const availableCount = document.querySelectorAll('.transfer-item:not(.hidden)').length;
    const cancelledCount = cancelledTransfers.size;
    
    document.getElementById('availableTransfersCount').textContent = availableCount;
    document.getElementById('cancelledTransfersCount').textContent = cancelledCount;
}

// Función para cargar la lista de traslados con la nueva estructura
function loadTransferList() {
    ...
    if (!processedData || processedData.length === 0) {
        transferList.innerHTML = `
            <div class="empty-state">
                <i class="codicon codicon-list-filter"></i>
                <h5>Sin datos procesados</h5>
                <p>Procesa un archivo CSV primero para ver los traslados</p>
            </div>
        `;
    ...
    if (transfers.length === 0) {
        transferList.innerHTML = `
            <div class="empty-state">
                ...
                <p>No hay traslados disponibles para anular en los datos procesados</p>
            </div>
        `;
    } else {
        // Crear tabla para mostrar los traslados
        let tableHTML = `
            <table>
                <thead>
                    <tr>
                        <th><input type="checkbox" id="selectAllTransfers" onclick="toggleAllTransfers()"></th>
                        <th>Traslado</th>
                        <th>OP</th>
                        <th>Referencia</th>
                        <th>Usuario</th>
        ...
        transfers.forEach(transfer => {
            tableHTML += `
                <tr class="transfer-item">
                    <td><input type="checkbox" value="${transfer.clave}" class="transfer-checkbox"></td>
                    <td><strong>${transfer.numero}</strong></td>
                    <td>${transfer.op}</td>
                    <td>${transfer.referencia}</td>
                    <td>${transfer.usuario}</td>
                    <td>${transfer.fecha}</td>
                    <td>${transfer.cantidad} unidades</td>
                </tr>
            `;
        });
        
        tableHTML += '</tbody></table>';
        transferList.innerHTML = tableHTML;
    }
...
function cancelSelectedTransfers() {
    ...
    showMessage(`${transfersToCancel.length} traslados anulados correctamente`, 'success', 3000);
    
    // Actualizar la interfaz
    updateCancelledTransfersTable();
    document.getElementById('reprocessBtn').style.display = 'inline-flex';
    document.getElementById('cancelledTransfersPanel').style.display = 'block';
...
function updateCancelledTransfersTable() {
    const tableBody = document.querySelector('#cancelledTransferTable tbody');
    ...
    cancelledTransfers.forEach(traslado => {
        tableHTML += `
            <tr>
                <td>${traslado}</td>
                <td><span style="color: var(--error); font-weight: 500;">Anulado</span></td>
                <td>
...
function restoreTransfer(traslado) {
    ...
    if (cancelledTransfers.size === 0) {
        ...
        document.getElementById('cancelledTransfersPanel').style.display = 'none';
    }
}

function reprocessCancelledTransfers() {
    if (cancelledTransfers.size === 0) {
        showMessage('No hay traslados anulados para reprocesar', 'info', 2000);
        return;
    }

    const confirmed = confirm(`¿Restaurar todos los ${cancelledTransfers.size} traslados anulados?`);
    ...
    const fileInput = document.getElementById('csvFile');
    if (fileInput.files.length > 0) {
        updateStatus('Reprocesando CSV con traslados restaurados...', 'loading');
        setTimeout(() => {
            processCSV();
        }, 1000);
    }
...
function filterTransfers() {
    ...
    const transferRows = document.querySelectorAll('#transferList tbody tr.transfer-item');
    ...
    // Actualizar contador
    document.getElementById('availableTransfersCount').textContent = foundCount;
...
function generateDistributionResultsHTML(recData) {
    ...
    html += '<table class="distribution-table"><thead><tr>';
