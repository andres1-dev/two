...
const Swal = {
    fire: function (options) {
        if (options.icon === 'info') {
            ...
            if (confirm(options.title + ': ' + options.text + ' ¿Continuar?')) {
                if (options.confirmButtonText === 'Entendido') {
                    // No action needed
                }
            }
        } else {
            alert(options.title + (options.text ? ': ' + options.text : ''));
        }
    }
};
...
function initializeDistribution() {
    // Mostrar loading del nuevo diseño
...
// Funciones adicionales para el nuevo diseño
...
function generateEmpresasUI() {
    ...
    row.className = 'empresas-container'; // Use a custom class or just rely on flex styles
    ...
    empresasData.forEach(([id, config]) => {
        ...
        container.appendChild(item); // Append directly to container
    });
}
...
// Versión compacta - todo en una fila
activeMayoristas.forEach(mayorista => {
    html += `
        <th>
...
function openFilterModal(event) {
    ...
    // Crear modal con el nuevo estilo
