...
const SoportesGrid = {
  ...
  init: async function () {
    ...
    this.kpiFacturasVal = document.getElementById('kpiFacturasVal');
    this.kpiFacturasBar = document.getElementById('kpiFacturasBar');
    this.kpiFacturasPct = document.getElementById('kpiFacturasPct');
    this.kpiUnidadesVal = document.getElementById('kpiUnidadesVal');
    this.kpiUnidadesBar = document.getElementById('kpiUnidadesBar');
    this.kpiUnidadesPct = document.getElementById('kpiUnidadesPct');
    this.kpiValoresVal = document.getElementById('kpiValoresVal');
    this.kpiValoresBar = document.getElementById('kpiValoresBar');
  ...
  cargarDatosKPI: async function () {
    if (!this.datosFacturadosCache) {
      this.kpiContainer.style.opacity = '0.5';
      try {
      ...
      this.kpiContainer.style.opacity = '1';
    }
  ...
  updateKPIs: async function (fechaInicio, fechaFin) {
    ...
    this.kpiContainer.style.display = 'grid'; // Mostrar KPIs cuando esté listo

    let start = fechaInicio;
    let end = fechaFin;

    // Normalizar si son objetos Date
    ...
    // Asegurar que end es final del dia
      ...
      data.forEach(doc => {
        if (doc.datosSiesa && Array.isArray(doc.datosSiesa)) {
          doc.datosSiesa.forEach(fact => {
            ...
            // Si la fecha de la factura cae en el rango solicitado:
            if (fd >= start && fd <= end) {
              stats.tFacturas++;

              const qty = parseFloat(fact.cantidad) || 0;
              const val = parseFloat(fact.valorBruto) || 0;

              stats.tUnidades += qty;
              stats.tValor += val;

              if (fact.confirmacion === 'ENTREGADO' || fact.confirmacion === 'ENTREGADO, PENDIENTE FACTURA') {
                stats.eFacturas++;
                stats.eUnidades += qty;
                stats.eValor += val;
              }
    ...
    const formatPesos = (n) => '$' + Math.round(n).toLocaleString('es-CO');
    const calcPct = (p, total) => total > 0 ? Math.round((p / total) * 100) : 0;

    const pFact = calcPct(stats.eFacturas, stats.tFacturas);
    const pUnds = calcPct(stats.eUnidades, stats.tUnidades);
    const pVals = calcPct(stats.eValor, stats.tValor);

    // Render Facts
    if (this.kpiFacturasVal) this.kpiFacturasVal.textContent = `${stats.eFacturas} / ${stats.tFacturas}`;
    if (this.kpiFacturasBar) this.kpiFacturasBar.style.width = `${pFact}%`;
    if (this.kpiFacturasPct) this.kpiFacturasPct.textContent = `${pFact}%`;

    // Render Units
    if (this.kpiUnidadesVal) this.kpiUnidadesVal.textContent = `${stats.eUnidades} / ${stats.tUnidades}`;
    if (this.kpiUnidadesBar) this.kpiUnidadesBar.style.width = `${pUnds}%`;
    if (this.kpiUnidadesPct) this.kpiUnidadesPct.textContent = `${pUnds}%`;

    // Render Valores
    if (this.kpiValoresVal) this.kpiValoresVal.textContent = `${formatPesos(stats.eValor)} / ${formatPesos(stats.tValor)}`;
    if (this.kpiValoresBar) this.kpiValoresBar.style.width = `${pVals}%`;
    if (this.kpiValoresPct) this.kpiValoresPct.textContent = `${pVals}%`;
  },
  ...
  downloadReport: async function () {
    ...
    try {
      ...
      // Agregar BOM para Excel en UTF-8 y encabezados
      let csvContent = '\uFEFF' + encabezados.join(';') + '\n';

      datos.forEach(doc => {
        if (doc.datosSiesa && Array.isArray(doc.datosSiesa)) {
          doc.datosSiesa.forEach(factura => {
            // Preparar fila asegurando formato CSV correcto
            const row = [
              factura.factura || '',
              factura.estado || '',
              factura.fecha || '',
              (factura.cliente || '').replace(/;/g, ','), // Reemplazar ; por , para no romper CSV
              factura.nit || '',
              factura.lote || '',
              factura.referencia || '',
              factura.cantidad || 0,
              factura.valorBruto || 0,
              factura.confirmacion || 'PENDIENTE',
              factura.fechaEntrega || '',
              factura.Ih3 || '',
              doc.documento || '',
              (factura.proovedor || '').replace(/;/g, ',')
            ];
      ...
      console.log('Reporte descargado:', filas.length, 'filas generadas');
