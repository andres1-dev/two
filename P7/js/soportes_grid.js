...
const SoportesGrid = {
  ...
  updateKPIs: async function (fechaInicio, fechaFin) {
    ...
    // Asegurar que end es final del día
    end.setHours(23, 59, 59, 999);

    const data = await this.cargarDatosKPI();

    // ===========================================
    // NUEVO: Crear mapa de facturas únicas
    // ===========================================
      ...
      data.forEach(doc => {
        if (doc.datosSiesa && Array.isArray(doc.datosSiesa)) {
          doc.datosSiesa.forEach(fact => {
            ...
            // Ignorar facturas vacías
            if (!facturaId || facturaId.trim() === '') return;

            // Si la factura NO existe en el mapa, la agregamos
    ...
    console.log(`KPIs: ${facturasUnicasMap.size} facturas únicas encontradas`);

    // ===========================================
    // Calcular estadísticas sobre facturas únicas
    // ===========================================
    let stats = {
      tFacturas: 0,
      eFacturas: 0,
    ...
    // Convertir el mapa a array para iterar
    ...
    console.log('📊 KPIs Calculados:', {
      facturas: `${stats.eFacturas} / ${stats.tFacturas} (${pFact}%)`,
      unidades: `${stats.eUnidades} / ${stats.tUnidades} (${pUnds}%)`,
      valor: `${formatPesos(stats.eValor)} / ${formatPesos(stats.tValor)} (${pVals}%)`
    });

    // Render Facturas
    if (this.kpiFacturasVal) {
      this.kpiFacturasVal.textContent = `${stats.eFacturas} / ${stats.tFacturas}`;
    }
    if (this.kpiFacturasBar) {
      this.kpiFacturasBar.style.width = `${pFact}%`;
    }
    if (this.kpiFacturasPct) {
      this.kpiFacturasPct.textContent = `${pFact}%`;
    }

    // Render Unidades
    if (this.kpiUnidadesVal) {
      this.kpiUnidadesVal.textContent = `${stats.eUnidades} / ${stats.tUnidades}`;
    }
    if (this.kpiUnidadesBar) {
      this.kpiUnidadesBar.style.width = `${pUnds}%`;
    }
    if (this.kpiUnidadesPct) {
      this.kpiUnidadesPct.textContent = `${pUnds}%`;
    }

    // Render Valores
    if (this.kpiValoresVal) {
      this.kpiValoresVal.textContent = `${formatPesos(stats.eValor)} / ${formatPesos(stats.tValor)}`;
    }
    if (this.kpiValoresBar) {
      this.kpiValoresBar.style.width = `${pVals}%`;
    }
    if (this.kpiValoresPct) {
      this.kpiValoresPct.textContent = `${pVals}%`;
    }
  },
