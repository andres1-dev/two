...
async function loadDataFromSheets() {
    ...
    try {
        ...
        // 2. Una vez cargados los clientes, inicializar distribución e impresión
        console.log('Datos base cargados, inicializando distribución e impresión...');

        // Ejecutar en paralelo distribución e impresión
        await Promise.all([
            initializeDistribution(),
            print_cargarDatos().catch(err => {
                console.error("Error cargando módulo de impresión:", err);
                // No bloqueamos la carga general si falla impresión
            })
        ]);
...
function getRepresentativeItem(items) {
    ...
    // Prioritize item with BODEGA === 'PRIMERAS' (which corresponds to 'DI' in CSV headers/bodegasMap)
    const itemPrimeras = items.find(item => item.BODEGA === 'PRIMERAS');
    // If found, return it. Otherwise, return the first item as fallback.
...
function setupPendientesSection(pendientes) {
    ...
    pendientes.forEach(item => {
        if (!opGroups[item.OP]) {
            opGroups[item.OP] = {
                // Inicializamos datos básicos, pero luego buscaremos el item representativo
                cantidad: 0,
    ...
    Object.keys(opGroups).forEach(op => {
        ...
        // Usar item representativo (PRIMERAS/DI) para la información mostrada
        const itemPrincipal = getRepresentativeItem(grupo.items);
        const primerItem = grupo.items[0]; // Mantenemos referencia al primero por si acaso, pero usamos principal para info

        const tieneSispro = sisproMap.has(op.trim());
        // Verificamos si CUALQUIER item tiene color válido, o si el principal tiene
        ...
        if (!tieneSispro || !tieneColor) {
            ...
            // Mostrar información del item principal (DI/PRIMERAS)
...
function loadOPData() {
    ...
    // Use representative item for details
...
function generateJSONForOP() {
    ...
    // Use OP from representative item
...
// Generic Wrapper for JSON Generation from Editor
            ...
            if (xhr.status === 200) {
                try {
                    ...
                    if (response.success) {
                        ...
                        // Clear Editor
                        if (document.getElementById('opEditorContainer')) {
                            document.getElementById('opEditorContainer').style.display = 'none';
                            document.getElementById('opEditorEmptyState').style.display = 'block';
                        }
...
function loadOPEditor() {
    ...
    if (document.getElementById('opEditorContainer')) {
        document.getElementById('opEditorContainer').style.display = 'block';
        document.getElementById('opEditorEmptyState').style.display = 'none';

        // Summary Header
        const primerItem = getRepresentativeItem(currentOPData);
        const infoDiv = document.getElementById('opEditorInfo');

        // Generate options for global user select
        const usuarioOptions = Object.values(escanersMap).map(fullname => {
            const isSelected = (primerItem.USUARIO === fullname);
            return `<option value="${fullname}" ${isSelected ? 'selected' : ''}>${fullname}</option>`;
        }).join('');

        // Handle valid user not in map
        let customUserOption = '';
        const isUserInMap = Object.values(escanersMap).includes(primerItem.USUARIO);
        if (!isUserInMap && primerItem.USUARIO) {
            customUserOption = `<option value="${primerItem.USUARIO}" selected>${primerItem.USUARIO}</option>`;
        }
        ...
        if (infoDiv) {
            infoDiv.innerHTML = `
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
                    ...
                    <div class="info-group">
                        <label style="font-size: 11px; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.5px;">Usuario (Global)</label>
                        <select class="form-control" style="padding: 2px 4px; height: 28px; font-size: 13px;" onchange="updateGlobalUser(this.value)">
                            ${customUserOption}
                            ${usuarioOptions}
                        </select>
                    </div>
...
function renderEditorTable() {
    ...
    currentOPData.forEach((item, index) => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <select class="form-control" style="padding: 4px; height: 30px; font-size: 13px;" onchange="handleEditorChange(${index}, 'BODEGA', this.value)">
                    ${bodegaOptions}
                </select>
            </td>
            <td>
                <input type="number" class="form-control" style="padding: 4px; height: 30px; width: 100px;" 
                       value="${item.CANTIDAD}" min="0" onchange="handleEditorChange(${index}, 'CANTIDAD', this.value)">
            </td>
            <td>
                <input type="number" class="form-control" style="padding: 4px; height: 30px; width: 120px;" 
                       value="${item.COSTO}" min="0" onchange="handleEditorChange(${index}, 'COSTO', this.value)">
            </td>
            <td><span style="font-family: monospace; font-size: 12px;">${item.TRASLADO}</span></td>
            <td>
                <button class="btn-icon" style="color: #f44747;" onclick="deleteEditorRow(${index})" title="Eliminar fila">
...
function handleEditorChange(index, field, value) {
    if (index >= 0 && index < currentOPData.length) {
        ...
        if (field === 'CANTIDAD') {
            ...
            // This is kept for individual row changes if needed, but UI now favors global
            item.USUARIO = value;
        }
...
function updateGlobalUser(newUsuario) {
    if (!currentOPData) return;

    // Update ALL items with new user
    currentOPData.forEach(item => {
        item.USUARIO = newUsuario;
    });

    updateStatus(`Usuario actualizado a: ${newUsuario} para toda la OP`, 'success');
}
...
function updateEditorStats() {
    ...
    const totalCost = currentOPData.reduce((sum, item) => sum + ((parseInt(item.CANTIDAD) || 0) * (parseFloat(item.COSTO) || 0)), 0);
...
function prepareDistributionDataFormat() {
    ...
    // Obtener todos los inputs de empresas para calcular totales
    const empresaInputs = document.querySelectorAll('#empresasContainer input[type="number"]');
    let totalPorcentaje = 0;

    // Calcular el porcentaje total asignado
    empresaInputs.forEach(input => {
        if (input.value) {
            totalPorcentaje += parseInt(input.value) || 0;
        }
    });
    ...
    // 1. Procesar empresas como Clientes principales
    // Primero, calcular porcentaje para la empresa principal (resta)
    ...
    // Restar porcentajes de empresas secundarias
    empresasSecundarias.forEach(([id, config]) => {
        const input = document.getElementById(`empresa-${id}`);
        if (input && input.value) {
            porcentajePrincipal -= parseInt(input.value) || 0;
        }
    });
    ...
    // Ahora procesar cada empresa con porcentajes corregidos
    empresasData.forEach(([id, config]) => {
        const clienteNombre = config.nombreCorto;

        // Determinar porcentaje correcto
        let porcentajeCliente;
        if (config.tipoEmpresa === "Principal") {
            porcentajeCliente = porcentajePrincipal;
        } else {
            const input = document.getElementById(`empresa-${id}`);
            porcentajeCliente = input ? parseInt(input.value) || 0 : 0;
        }

        // Obtener total de esta empresa
        let totalEmpresa = 0;

        if (config.tipoEmpresa === "Principal") {
            const totalElement = document.querySelector('.principal-total');
            totalEmpresa = parseInt(totalElement?.textContent) || 0;
        } else {
            const totalElement = document.querySelector(`.secundaria-total[data-empresa="${id}"]`);
            totalEmpresa = parseInt(totalElement?.textContent) || 0;
        }

        // Si tiene asignación, procesar detalle por fila
        if (totalEmpresa > 0 && currentRecData.HR) {
            ...
            currentRecData.HR.forEach((row, rowIndex) => {
                ...
                // Obtener cantidad asignada a esta empresa
                ...
                if (cantidadEmpresa > 0) {
                    distribucion.push({
                        ...
                        color: color,
                        talla: talla,
                        cantidad: cantidadEmpresa
                    });
                }
            });

            if (distribucion.length > 0) {
                // Solo incluir porcentaje si es mayor que 0
                const clienteData = {
                    id: id,
                    distribucion: distribucion
                };

                if (porcentajeCliente > 0) {
                    clienteData.porcentaje = `${porcentajeCliente}%`;
                }
    ...
    // 2. Procesar mayoristas como Clientes adicionales
    activeMayoristas.forEach(mayorista => {
        const totalElement = document.querySelector(`.mayorista-total[data-mayorista="${mayorista.id}"]`);
        ...
        if (totalMayorista > 0 && currentRecData.HR) {
            ...
            currentRecData.HR.forEach((row, rowIndex) => {
                ...
                // Obtener cantidad para este mayorista
                ...
                if (cantidadMayorista > 0) {
                    distribucion.push({
                        ...
                        color: color,
                        talla: talla,
                        cantidad: cantidadMayorista
                    });
                }
            });

            if (distribucion.length > 0) {
                // Mayoristas no llevan porcentaje
                distributionData.Clientes[mayorista.nombre] = {
                    id: mayorista.id,
                    distribucion: distribucion
                };
            }
    ...
    console.log('Datos preparados en formato específico:', distributionData);
...
function previewDistributionFormat() {
    ...
    // Mostrar en consola
    console.log('Preview del formato JSON:', JSON.stringify(data, null, 2));

    // Mostrar en modal para revisión
    createModal('Previsualización del Formato', `
        <div style="max-height: 500px; overflow-y: auto;">
                ...
                <ul>
                    ${Object.entries(data.Clientes).map(([nombre, cliente]) => `
                        <li>
                            ...
                            <br>
                            Porcentaje: ${cliente.porcentaje || 'N/A'}
                            <br>
                            Items: ${cliente.distribucion ? cliente.distribucion.length : 0}
                        </li>
                    `).join('')}
                </ul>
            </div>
            ...
            <div style="background: var(--sidebar); padding: 12px; border-radius: 6px; margin-top: 20px;">
                ...
                <pre style="font-size: 10px; overflow-x: auto; white-space: pre-wrap; word-wrap: break-word;">
${JSON.stringify(data)}
                </pre>
            </div>
...
// Agregar botón de previsualización a la interfaz
...
// Actualizar setupDistributionEventListeners
function setupDistributionEventListeners() {
    // Botón de recarga
    const reloadBtn = document.getElementById('reloadDataBtn');
    if (reloadBtn) {
        reloadBtn.addEventListener('click', reloadAllDistributionData);
    }

    // Botón de guardar - CONECTAR A LA NUEVA FUNCIÓN
    const saveBtn = document.getElementById('saveDistributionsBtn');
    if (saveBtn) {
    ...
    if (configBtn) {
        configBtn.addEventListener('click', showModuleSettings);
    }

    // Agregar botón de previsualización
    addPreviewButton();
...
// CSS adicional para el estado de éxito
const successCSS = `
.success-state {
    text-align: center;
    padding: 40px 20px;
...
if (!document.getElementById('success-css')) {
    const style = document.createElement('style');
...
// Llamar a addPreviewButton cuando se active la pestaña
document.addEventListener('DOMContentLoaded', function () {
    // Escuchar cambios en las pestañas
    document.querySelectorAll('[data-tab="distribution"]').forEach(element => {
