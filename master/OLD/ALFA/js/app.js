...
function loadOPData() {
    ...
    // Reset other fields to defaults or empty
    const fieldsToReset = ['proveedor', 'auditor', 'gestor'];
    fieldsToReset.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = el.querySelector('option[selected]') ? el.querySelector('option[selected]').value : '';
    });
    ...
    // Redirect to Editor Tab immediately
    const editorTabBtn = document.querySelector('.activity-icon[data-tab="op-editor"]') || document.querySelector('.tab[data-tab="op-editor"]');
    if (editorTabBtn) {
        editorTabBtn.click();
    }
...
const DISTRIBUTION_API_KEY = DIS_API_KEY; // Alias for compatibility
...
// Constantes para distribución (Movidas al inicio del módulo)
// Ver líneas ~2482
