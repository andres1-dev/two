...
function smartDistribution(availableRows, totalQty) {
    ...
    console.log(`Ejecutando distribución con estrategia: ${strategy} para ${totalQty} unidades`);
 ...
 * Cantidad Requerida / Cantidad de items disponibles
 */
function distributeEqual(availableRows, totalQty) {
    ...
    let remainder = totalQty % itemCount;
    ...
    // Recalcular el remainder real basado en lo que no se pudo asignar por tope de stock
    let totalAssigned = distributed.reduce((sum, item) => sum + item.assigned, 0);
    let remainingToDistribute = totalQty - totalAssigned;

    if (remainingToDistribute > 0) {
        // Repartir el sobrante de a 1 unidad priorizando donde hay más stock restante
        distributed.sort((a, b) => (b.tdm - b.assigned) - (a.tdm - a.assigned));
        for (let i = 0; i < distributed.length && remainingToDistribute > 0; i++) {
            const availableSpace = distributed[i].tdm - distributed[i].assigned;
            if (availableSpace > 0) {
 ...
 * Equivalente ponderado con respecto a la cantidad disponible por item
 */
function distributeWeighted(availableRows, totalQty) {
    ...
    let distributed = availableRows.map(row => {
        ...
        let assigned = Math.floor(weight * totalQty);
        assigned = Math.min(assigned, row.tdm);
        remaining -= assigned;
        return { ...row, assigned };
    });

    // Ajuste de decimales (sobrantes) por prioridad de stock
    if (remaining > 0) {
        distributed.sort((a, b) => (b.tdm - b.assigned) - (a.tdm - a.assigned));
        for (let i = 0; i < distributed.length && remaining > 0; i++) {
            const space = distributed[i].tdm - distributed[i].assigned;
            if (space > 0) {
                distributed[i].assigned += 1;
                remaining -= 1;
            }
...
function distributeSectorized(availableRows, totalQty) {
    // 1. Identificar items Core
    const coreRows = availableRows.filter(row => CORE_SIZES.includes(row.talla));

    // Si no hay tallas core, usamos ponderada normal
    if (coreRows.length === 0) return distributeWeighted(availableRows, totalQty);

    // 2. Realizar una distribución ponderada base con un multiplicador mayor para el Core
    const weightMultiplier = 2.0; // Las tallas core pesan el doble en el reparto inicial

    let weightedTotal = 0;
    availableRows.forEach(row => {
        const isCore = CORE_SIZES.includes(row.talla);
        weightedTotal += row.tdm * (isCore ? weightMultiplier : 1.0);
    });

    let remaining = totalQty;
    let finalDistribution = availableRows.map(row => {
        const isCore = CORE_SIZES.includes(row.talla);
    ...
    // 3. Si sobran unidades por redondeo, se van TODAS al Core primero
    if (remaining > 0) {
        // Ordenar coreRows por stock disponible descendente
        finalDistribution.sort((a, b) => {
            if (a.isCore && !b.isCore) return -1;
            if (!a.isCore && b.isCore) return 1;
            return (b.tdm - b.assigned) - (a.tdm - a.assigned);
        });

        for (let i = 0; i < finalDistribution.length && remaining > 0; i++) {
            const item = finalDistribution[i];
            const space = item.tdm - item.assigned;
            if (space > 0) {
                const anyCoreLeft = finalDistribution.some(f => f.isCore && (f.tdm - f.assigned) > 0);
                if (item.isCore || !anyCoreLeft) {
                    item.assigned += 1;
                    remaining -= 1;
                }
            }
...
async function saveDistributionToSheets() {
    ...
    try {
        ...
        if (verification.success && verification.verified) {
            ...
            // --- NUEVO: Recargar datos del módulo de impresión ---
            console.log('🖨️ Actualizando datos del módulo de impresión...');
            try {
                if (typeof window.print_cargarDatos === 'function') {
                    await window.print_cargarDatos();
                    console.log('✅ Datos de impresión actualizados');
                } else {
                    console.warn('⚠️ Función print_cargarDatos no encontrada');
                }
            } catch (printError) {
                console.error('Error actualizando datos de impresión:', printError);
                // No detenemos el flujo principal, solo lo registramos
            }
            // --- FIN NUEVO ---
    ...
    createModal('Configuración del Módulo de Distribución', `
        <div style="padding: 16px 0;">
            ...
            <div style="display: flex; gap: 8px; justify-content: flex-end; margin-top: 24px;">
                ...
                <button class="btn-primary" onclick="saveModuleSettings()">Guardar Configuración</button>
            </div>
        </div>
    `, true);
}
...
function saveModuleSettings() {
    ...
    // Aplicar al select actual si existe
    ...
    // Si hay datos cargados, recalcular
    if (currentRecData) {
...
function setupDistributionEventListeners() {
    ...
    if (strategySelect) {
        ...
        strategySelect.addEventListener('change', () => {
            console.log('Estrategia cambiada, actualizando distribuciones activas...');
            recalculateActiveDistributions();
        });
    }
