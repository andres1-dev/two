...
function initUIListeners() {
    ...
    // Foco persistente excepto cuando la cámara está abierta o estamos en login
    function enforceFocus() {
        ...
        // Si el login es visible, NO forzar foco al barcode
        if (loginScreen && getComputedStyle(loginScreen).display !== 'none') {
            return;
        }

        // Solo aplicar foco si la cámara no está abierta y no estamos escribiendo en login
        if (document.activeElement !== barcodeInput &&
            cameraModal && cameraModal.style.display !== 'flex' &&
            document.activeElement !== loginId &&
            document.activeElement !== loginPass) {
