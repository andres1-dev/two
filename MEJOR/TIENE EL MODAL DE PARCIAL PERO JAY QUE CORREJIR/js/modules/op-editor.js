// ============================================
// ESTADO LOCAL Y AYUDANTES PARA PARCIALES
// ============================================

let lastPartialsSummary = null;
...
/**
 * Consulta el historial de parciales de una OP y actualiza el panel del editor
 */
async function checkPartialsForOPEditor(opNumber) {
    const summaryDiv = document.getElementById('opEditorPartialSummary');
    const section = document.getElementById('opEditorPartialSection');
    const checkbox = document.getElementById('checkPartialDelivery');

    if (!summaryDiv || !section) return;

    // Resetear
    lastPartialsSummary = null;
    if (checkbox) checkbox.checked = false;
    section.style.display = 'none';

    try {
        summaryDiv.innerHTML = '<span class="loading-spinner"></span> Consultando historial...';
        lastPartialsSummary = await getOPPartialsSummary(opNumber);

        if (lastPartialsSummary && (lastPartialsSummary.partials.length > 0 || lastPartialsSummary.totalAlreadySaved > 0)) {
            section.style.display = 'block';
            updatePartialSummaryUI();
        }
    } catch (error) {
        console.error('Error al verificar parciales:', error);
        summaryDiv.innerHTML = '<i class="codicon codicon-warning" style="color: var(--error);"></i> Error al consultar historial';
    }
}

/**
 * Actualiza el texto informativo de parciales en el panel
 */
function updatePartialSummaryUI() {
    const summaryDiv = document.getElementById('opEditorPartialSummary');
    const section = document.getElementById('opEditorPartialSection');
    if (!summaryDiv || !lastPartialsSummary) return;

    const primerItem = getRepresentativeItem(currentOPData);
    const totalOP = primerItem?.TOTAL || primerItem?.CANTIDAD_TOTAL || 0;
    const actualUnits = currentOPData.reduce((sum, item) => sum + (parseInt(item.CANTIDAD) || 0), 0);
    const totalPrevio = lastPartialsSummary.totalAlreadySaved;
    const totalAcumulado = totalPrevio + actualUnits;

    const faltanteTotal = totalOP - totalAcumulado;

    if (totalPrevio > 0 || (totalOP > 0 && actualUnits < totalOP)) {
        section.style.display = 'block';

        let html = `Hay ${lastPartialsSummary.partials.length} entregas previas (${totalPrevio.toLocaleString('es-CO')} unds). `;
        html += `Acumulado: <strong>${totalAcumulado.toLocaleString('es-CO')} / ${totalOP.toLocaleString('es-CO')}</strong>`;

        if (faltanteTotal > 0) {
            html += ` <span style="color: var(--warning); font-weight: 600;">(Faltan ${faltanteTotal.toLocaleString('es-CO')})</span>`;
        } else if (faltanteTotal === 0) {
            html += ` <span style="color: var(--success); font-weight: 600;">(¡Lote Completo!)</span>`;
        } else {
            html += ` <span style="color: var(--info); font-weight: 600;">(Excedente: ${Math.abs(faltanteTotal).toLocaleString('es-CO')})</span>`;
        }
...
/**
 * Manejador del checkbox de parciales
 */
window.handlePartialToggle = function () {
    const isChecked = document.getElementById('checkPartialDelivery')?.checked;
    console.log('🔄 Despacho parcial seleccionado:', isChecked ? 'SÍ' : 'NO');

    // Regenerar el JSON para que el usuario vea el cambio de nombre (A)
    generateJSONFromEditor();
};
...
async function loadOPData() {
    ...
    // NUEVO: Consultar historial de parciales
    await checkPartialsForOPEditor(primerItem.OP);
...
function updateEditorStats() {
    ...
    // NUEVO: Actualizar el resumen de parciales si el panel está visible
    updatePartialSummaryUI();
}
...
function generateJSONForOP() {
    ...
    // --- NUEVO: Lógica de Sufijo para Parciales ---
    const isPartial = document.getElementById('checkPartialDelivery')?.checked;
    let opName = primerItem.OP;

    if (isPartial) {
        const baseOP = String(opName).split(' ')[0];
        let nextSuffix = 1;

        if (lastPartialsSummary && lastPartialsSummary.partials.length > 0) {
            const suffixes = lastPartialsSummary.partials.map(p => {
                // Soportar con o sin espacio para detectar existentes
                const match = String(p.op).match(/[ E|E](\d+)$/);
        ...
        opName = `${baseOP}E${nextSuffix}`;
        console.log(`📝 Nombre de OP ajustado para despacho parcial: ${opName}`);
    }
...
async function saveToSheets() {
    ...
    // --- NUEVO: Validar Parciales ---
    const loadingPartials = showQuickLoading('Validando parciales existentes...');
    const partialsSummary = await getOPPartialsSummary(jsonData.A);
    loadingPartials.close();

    const totalPrevio = partialsSummary.totalAlreadySaved;
    const totalActual = jsonData.TOTAL_GENERAL || 0;
    const totalAcumulado = totalPrevio + totalActual;
    const totalOP = jsonData.CANTIDAD || 0;
    ...
    if (totalPrevio > 0) {
        footerHTML = `
            <tr style="border-top: 1px solid var(--border); background: rgba(0,0,0,0.02);">
                <td style="text-align: center; padding: 10px; color: var(--text-secondary); font-size: 12px;">PARCIALES ENCONTRADOS</td>
                <td style="text-align: center; padding: 10px; color: var(--text-secondary); font-size: 13px; font-weight: 600;">${totalPrevio.toLocaleString('es-CO')} unds</td>
            </tr>
            <tr style="border-top: 2px solid var(--border); background: rgba(0, 120, 212, 0.05);">
                <td style="text-align: center; padding: 12px; color: var(--primary); font-weight: 700; font-size: 13px;">TOTAL ACUMULADO</td>
                <td style="text-align: center; padding: 12px; color: var(--primary); font-weight: 700; font-size: 15px;">${totalAcumulado.toLocaleString('es-CO')} / ${totalOP.toLocaleString('es-CO')}</td>
            </tr>
        `;
    }
    ...
    const confirmed = await showQuickConfirm(
        ...
        diferencia > 0
            ? `Hay <strong>${diferencia} unidades faltantes</strong> en esta orden de producción.<br><br>¿Deseas proceder con el guardado?`
            : `La orden de producción está <strong>completa</strong>.<br><br>¿Deseas guardar los datos en Google Sheets?`,
