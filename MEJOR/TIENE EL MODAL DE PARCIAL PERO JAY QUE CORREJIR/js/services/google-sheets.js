...
async function fetchSheetData(range) {
    // Fetch desde API - Caché elminada por solicitud del usuario
...
/**
 * Busca todos los parciales de una OP en la hoja DATA2 (Columna S tiene el JSON)
 */
async function getOPPartialsSummary(baseOP) {
    try {
        console.log(`🔍 Buscando parciales para OP: ${baseOP}...`);
        // Nota: El JSON está en la columna S (índice 18, si A es 0)
        // Pero fetchSheetData('DATA2!S2:S') nos da solo esa columna, por lo que el índice será 0
        const data = await fetchSheetData('DATA2!S2:S');

        if (!data.values || data.values.length === 0) {
            return { totalAlreadySaved: 0, partials: [] };
        }

        let totalAlreadySaved = 0;
        const partials = [];
        const baseOPStr = String(baseOP).trim();

        data.values.forEach(row => {
            if (!row[0]) return;
            try {
                const jsonData = JSON.parse(row[0]);
                const op = String(jsonData.A || '').trim();

                // Expresión regular para coincidir con OP base o OP base + " E" + número
                // Ejemplo: 2345 coincide con 2345, 2345 E1, 2345 E2
                if (op === baseOPStr || op.startsWith(baseOPStr + ' E')) {
                    const qty = parseInt(jsonData.TOTAL_GENERAL) || parseInt(jsonData.CANTIDAD) || 0;
                    totalAlreadySaved += qty;
                    partials.push({ op, qty });
                }
            } catch (e) {
                // Fila no válida o no es JSON
            }
        });

        console.log(`📊 Parciales encontrados: ${partials.length}, Total acumulado: ${totalAlreadySaved}`);
        return { totalAlreadySaved, partials };
    } catch (error) {
        console.error('Error al obtener parciales:', error);
        return { totalAlreadySaved: 0, partials: [], error: error.message };
    }
}
...
async function saveNewColorData(data) {
    // Usamos el GAS dedicado que ahora maneja tanto OPs como Colores
    const formData = new URLSearchParams();
