...
async function processCSVData(rows) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (escanerCache.has(usuario)) {
            usuarioValido = escanerCache.get(usuario);
        } else {
            usuarioValido = escanersMap.has(usuario);
            escanerCache.set(usuario, usuarioValido);
        }
