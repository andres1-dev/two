    ...
    const procesamientos = recValues.map(async (row) => {
        try {
            ...
            if (tipo.toUpperCase() === "PROMO") {
                return await procesarPromoRecOptimizado(row, documento, fecha, lote, refprov, descripcion, referencia, tipo, pvp, prenda, genero, linea, cantidad, soportesData, siesaData, registros);
...
async function procesarPromoRecOptimizado(row, documento, fecha, lote, refprov, descripcion, referencia, tipo, pvp, prenda, genero, linea, cantidad, soportesData, siesaData, registros) {
    let documentoPrincipal = documento;
    
    const mismoLote = Number(row[8]) || 0;
    if (mismoLote > 0) {
        // Búsqueda optimizada en el mismo array
        const registroFullMismoLote = row._fullData?.find(r => {
