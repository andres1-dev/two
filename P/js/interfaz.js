            ...
            if (!adminSection) {
                ...
                adminSection.innerHTML = `
                    <div class="setting-info" style="margin-bottom: 12px;">
                        <span class="setting-title" style="color: var(--text-main); font-weight: 700;">
                            <i class="fas fa-shield-alt" style="color: var(--primary); margin-right: 8px;"></i>Administración
                        </span>
                        <span class="setting-desc">Panel de control de super usuario</span>
                    </div>
                    <div style="margin-bottom: 12px; display: flex; flex-direction: column; gap: 8px;">
                        <label for="reportDateInput" style="font-size: 0.75rem; color: var(--text-secondary); font-weight: 600;">Fecha del Reporte:</label>
                        <input type="date" id="reportDateInput" class="setting-input" style="width: 100%; border-radius: 10px; border: 1px solid var(--border); padding: 8px; font-family: inherit; font-size: 0.85rem; background: var(--background); color: var(--text-main);">
                    </div>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                        <button id="adminUsersBtn" class="btn-primary" style="width: 100%; display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px; border-radius: 12px; font-weight: 600; font-size: 0.85rem;">
                            <i class="fas fa-users-cog"></i> Usuarios
                        </button>
                        <button id="sendSummaryBtn" class="btn-primary" style="width: 100%; display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px; border-radius: 12px; font-weight: 600; font-size: 0.85rem; background: #059669;">
                ...
                // Re-vincular el evento del botón de reporte inyectado
                const summaryBtn = document.getElementById('sendSummaryBtn');
                if (summaryBtn && window.notificationManager) {
                    summaryBtn.onclick = (e) => {
                        e.preventDefault();
                        const selectedDate = document.getElementById('reportDateInput')?.value;
                        window.notificationManager.sendDailySummary(selectedDate);
                    };
                }
            }
