...
function setupTemplates() {
    if (!elements.templateChips) return;
    const chips = elements.templateChips.querySelectorAll('.template-chip');
    chips.forEach(chip => {
        chip.addEventListener('click', () => {
            ...
            if (elements.notifTitle) elements.notifTitle.value = title;
            if (elements.notifBody) elements.notifBody.value = body;
            // Actualizar preview
            updatePreview();
        });
    });
...
function updatePreview() {
    const title = elements.notifTitle?.value?.trim() || 'Título del mensaje';
    const body = elements.notifBody?.value?.trim() || 'El cuerpo del mensaje aparecerá aquí...';
    if (elements.previewTitle) elements.previewTitle.textContent = title;
    if (elements.previewBody) elements.previewBody.textContent = body;
}

function setupPreview() {
    if (elements.notifTitle) {
        elements.notifTitle.addEventListener('input', () => {
            // Desactivar chip activo al editar manualmente
            deactivateChips();
            updatePreview();
        });
    }
    if (elements.notifBody) {
        elements.notifBody.addEventListener('input', () => {
            deactivateChips();
            updatePreview();
        });
    }
...
function deactivateChips() {
    if (!elements.templateChips) return;
    elements.templateChips.querySelectorAll('.template-chip').forEach(c => c.classList.remove('active'));
}
...
async function sendNotification() {
    ...
    if (!title || !body) {
        alert('⚠️ Completa el título y el mensaje antes de enviar');
        return;
    }
    ...
    try {
        setStatus('🔄 Enviando notificación personalizada...');
        const result = await callGAS('send-notification', 'POST', {
            title, body,
            icon: location.origin + location.pathname.replace(/[^/]*$/, '') + 'icons/icon-192x192.png'
        });

        if (result && result.success) {
            const msg = '✅ ' + (result.message || 'Notificación enviada exitosamente');
            alert(msg);
            setStatus(msg);
            // Limpiar campos después de enviar exitosamente
            if (elements.notifTitle) elements.notifTitle.value = '';
            if (elements.notifBody) elements.notifBody.value = '';
            deactivateChips();
            updatePreview();
        } else {
        ...
        elements.sendBtn.textContent = '📨 Enviar Notificación a Todos';
    }
}
...
async function initialize() {
    try {
        ...
        // Configurar plantillas y vista previa
        setupTemplates();
        setupPreview();
        updatePreview();
